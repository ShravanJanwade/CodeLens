...
// The unified Hono API replaces the legacy Go gateway. Keeping `/api/v1` here
// preserves the extension's public client contract while it uses the new API.
    ...
    if (typeof chrome !== 'undefined' && chrome.storage) {
      try {
        ...
        if (result['codelens-storage']?.state?.settings?.apiUrl) {
          const savedUrl = result['codelens-storage'].state.settings.apiUrl;
          // Migrate the earlier extension default, which pointed at a gateway
          // port that is no longer part of the unified platform.
          this.setBaseUrl(savedUrl === 'http://localhost:8080' ? 'http://localhost:4000' : savedUrl);
        }
      } catch (error) {
