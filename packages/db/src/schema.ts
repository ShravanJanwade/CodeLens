...
export const repositories = sqliteTable(
  'repositories',
  {
    ...
    healthStatus: text('health_status', { enum: ['healthy', 'warning', 'critical', 'unknown'] })
      .notNull()
      .default('unknown'),
...
export const codeAnalysisRuns = sqliteTable(
  'code_analysis_runs',
  {
    id: text('id').primaryKey(),
...
export const codeFindings = sqliteTable(
  'code_findings',
  {
    ...
    analysisRunId: text('analysis_run_id')
      .notNull()
      .references(() => codeAnalysisRuns.id),
    repositoryId: text('repository_id')
      .notNull()
      .references(() => repositories.id),
...
export const repositoryFiles = sqliteTable(
  'repository_files',
  {
    id: text('id').primaryKey(),
    repositoryId: text('repository_id')
...
export const repositoryEdges = sqliteTable(
  'repository_edges',
  {
    id: text('id').primaryKey(),
    repositoryId: text('repository_id')
...
export const githubWebhookEvents = sqliteTable(
  ...
  (table) => ({
    idx_github_webhook_events_repo: index('idx_github_webhook_events_repo').on(table.repositoryId),
  }),
);
...
export const incidents = sqliteTable(
  'incidents',
  {
    ...
    status: text('status', {
      enum: ['open', 'investigating', 'mitigating', 'awaiting_approval', 'resolved', 'closed'],
    })
      .notNull()
      .default('open'),
    serviceId: text('service_id')
...
export const alerts = sqliteTable(
  'alerts',
  {
    ...
    status: text('status', { enum: ['firing', 'resolved', 'suppressed'] })
      .notNull()
      .default('firing'),
  },
  (table) => ({
...
export const hypotheses = sqliteTable(
  'hypotheses',
  {
    ...
    title: text('title').notNull(),
    description: text('description').notNull(),
    status: text('status', { enum: ['active', 'confirmed', 'eliminated', 'investigating'] })
      .notNull()
      .default('active'),
...
export const remediationPlans = sqliteTable(
  'remediation_plans',
  {
    ...
    incidentId: text('incident_id')
      ...
      .references(() => incidents.id),
    action: text('action', {
      enum: ['rollback', 'restart', 'scale', 'disable_feature', 'update_config', 'manual'],
    }).notNull(),
...
export const incidentEvidence = sqliteTable(
  'incident_evidence',
  {
    id: text('id').primaryKey(),
    incidentId: text('incident_id')
